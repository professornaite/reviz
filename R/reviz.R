  ...
  shiny::shinyApp(
    ui = shiny::fluidPage(
      ...
      shiny::sidebarLayout(
        shiny::sidebarPanel(
          ...
          shiny::selectInput("z", "Grouping Variable (optional)", choices = c("None" = ""), selected = ""),
          shiny::radioButtons(
            "z_mode",
            "Grouping Display",
    ...
    server = function(input, output, session) {
      ...
      shiny::observeEvent(input$data_pkg, {
        if (input$data_source == "builtin") {
          if (input$data_pkg == "base") {
            ...
            shiny::updateSelectInput(session, "dataset", choices = c("inequality_systems" = "inequality_systems"))
          } else if (input$data_pkg == "critstats") {
      ...
      shiny::observeEvent(get_data(), {
        ...
        z_choices <- c("None" = "")
        if (length(vars) > 0) {
          z_choices <- c(z_choices, setNames(vars, vars))
        }
        shiny::updateSelectInput(session, "z", choices = z_choices, selected = "")

        vars <- vars[!(vars %in% c("county"))]
        if (length(vars) >= 2) {
      ...
      output$plot <- shiny::renderPlot({
        ...
        z <- if (nzchar(input$z)) input$z else NULL

        p <- ggplot2::ggplot(data, ggplot2::aes(x = .data[[input$x]], y = .data[[input$y]])) +
          ggplot2::geom_point(alpha = 0.6, size = 2.5, color = "#2E86AB")

        if (!is.null(z)) {
          if (input$z_mode == "color") {
        ...
        if (input$model == "lm") {
          p <- p + ggplot2::geom_smooth(method = "lm", se = TRUE, linewidth = 1.1, color = "#A23B72")
        } else if (input$model == "loess") {
          p <- p + ggplot2::geom_smooth(method = "loess", se = TRUE, linewidth = 1.1, color = "#A23B72")
        ...
        p + ggplot2::theme_minimal(base_size = 13) +
          ggplot2::labs(
            ...
            subtitle = paste(
              "Source:",
              ifelse(input$data_source == "uploaded", "User Upload", paste(input$dataset, "(", input$data_pkg, ")")),
              "| N =",
              nrow(data)
            ),
            x = input$x,
      ...
      output$r_code <- shiny::renderText({
        ...
        data_comment <- if (input$data_source == "uploaded") {
        ...
        paste0(
          ...
          "p <- ggplot(data, aes(x = ", input$x, ", y = ", input$y, ")) +\n",
          "  geom_point(alpha = 0.6, size = 2.5, color = '#2E86AB') +\n",
          "  ", model_code, " +\n",
          "  theme_minimal(base_size = 13) +\n",
          "  labs(\n",
          "    title = '", input$y, " vs ", input$x, "',\n",
          "    x = '", input$x, "', y = '", input$y, "'\n",
          "  ) +\n",
